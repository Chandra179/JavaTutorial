...
public class Twootr {
	...
	public Optional<SenderEndPoint> onLogon(final String userId, final String password,
		...
		authenticatedUser.ifPresent(user -> {
			...
			twootRepository.query(
					new TwootQuery().inUsers(user.getFollowing()).lastSeenPosition(user.getLastSeenPosition()),
					user::receiveTwoot);
	...
	FollowStatus onFollow(final User follow, final String userIdToFollow) {
		return userRepository.get(userIdToFollow).map(userToFollow -> userRepository.follow(follow, userToFollow))
				.orElse(INVALID_USER);
	}
